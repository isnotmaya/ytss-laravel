...
<div class="space-y-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-white/50 mt-1">
                Agenda kegiatan, acara, dan jadwal operasional penting untuk kelompok kelas anak Anda.
            </p>
        </div>
        <div class="bg-orange-500/10 border border-orange-500/20 px-4 py-2.5 rounded-2xl self-start md:self-center flex items-center gap-2 text-orange-400">
            ...
            <span class="text-xs uppercase font-bold tracking-wider heading-font">
                {{ ($siswa && $siswa->kelompokKelas) ? $siswa->kelompokKelas->nama_kelompok : 'Umum' }}
            </span>
        </div>
    </div>
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between gap-2">
                            ...
                            <div class="text-xs text-white/40 font-medium">
                                {{ substr($agenda->jam_mulai, 0, 5) }} - {{ substr($agenda->jam_selesai, 0, 5) }} WIB
                            </div>
                        </div>
    ...
    @else
        ...
        <div class="text-center py-20 border border-dashed border-white/10 rounded-[32px] bg-white/[0.01]">
            <i data-lucide="book-open" class="w-16 h-16 text-white/20 mx-auto mb-4"></i>
            <h3 class="heading-font text-2xl font-bold uppercase tracking-wider text-white mb-2">Belum Ada Agenda</h3>
            <p class="text-sm text-white/50 max-w-md mx-auto leading-relaxed">
                Saat ini belum ada agenda kegiatan terdaftar untuk kelompok kelas anak Anda. Silakan cek berkala halaman ini untuk info mendatang.
            </p>
        </div>
    @endif
</div>
