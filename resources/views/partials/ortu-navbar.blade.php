@php
    $ortuModel = \App\Models\SiswaOrtu::where('kd_users', Auth::user()->kd_users)->first();
    $hasPhoto = $ortuModel && !empty($ortuModel->upload_foto) && file_exists(public_path($ortuModel->upload_foto));
@endphp
...
<header class="sticky top-0 z-30 bg-[#050505]/85 backdrop-blur-md border-b border-white/5 px-6 py-4 flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
        ...
        @if($hasPhoto)
            <img src="{{ asset($ortuModel->upload_foto) }}" 
                 class="w-10 h-10 rounded-full border border-white/10 object-cover bg-neutral-900 shadow-lg" 
                 alt="Avatar">
        @else
            <div class="w-10 h-10 rounded-full bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-500 font-bold text-sm uppercase heading-font shadow-lg">
